...
async function showMyRequests() {
    ...
    const { data, error } = await _supabase
        .from('requests')
        .select('created_at, issue, is_completed')
    ...
    listContainer.innerHTML = data.map(req => `
        <div class="request-card">
            ...
            <p><strong>Stato:</strong> 
                <span class="status ${req.is_completed ? 'completed' : 'pending'}">
                    ${req.is_completed ? 'Completato' : 'In Elaborazione'}
                </span>
            </p>
        </div>
    `).join('');
